...
- If there are **conditions or restrictions** (e.g., AI banned on a specific label such as `security`, AI allowed only with full human review): continue, note them.
...
- **Beginner-reserved label — unconditional hard stop.** Read the issue's labels from the `issue` command's `data.labels`. If ANY label is a newcomer-reserved label — `good first issue`, `good-first-issue`, `good_first_issue`, `beginner`, `first-timers-only`, `e-easy`, `level: starter` / `starter`, or any clear equivalent (match case-insensitively, ignoring space / hyphen / underscore separators) — **STOP. Do NOT write code for this issue, regardless of the project's AI policy.** This fires even when no AI policy exists or the policy would permit AI on the label. Then write `redirect_report.md` in the workspace root, containing: the beginner-reserved rationale (these are on-ramps reserved for human newcomers — framed as a deliberate stance, not "you can't"), alternative open issues the contributor CAN work on with AI, the project's AI disclosure format (or the voluntary template if none), and non-AI guidance for the original issue (approach hints from the issue, referenced code files, relevant conventions from recon). The file is the deliverable. See the rule "Never work on beginner-reserved issues — hard stop."
- **Other policy-restricted labels — conditional.** Cross-reference the issue's labels and assignment against any restrictions found in Step 2. If the AI policy restricts AI usage on a specific label *other than the beginner-reserved labels already handled above* (e.g., a project bans AI on `security` issues) and THIS issue carries it: **STOP, then write `redirect_report.md` in the workspace root** — containing the restriction (with the policy quote), why it exists, alternative open issues the contributor CAN work on with AI, and non-AI guidance for the restricted issue (approach hints from the issue, referenced code files, relevant conventions from recon). The file is the deliverable.
